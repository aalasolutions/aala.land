...
export class AddMissingColumnsToLeads1774000000007 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_LEADS_REGION_CODE" ON "leads"("region_code")`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "leads" DROP CONSTRAINT IF EXISTS "fk_leads_unit"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "leads" DROP COLUMN IF EXISTS "unit_id"`,
    );
    await queryRunner.query(
      `ALTER TABLE "leads" DROP COLUMN IF EXISTS "property_id"`,
    );
  }
}
