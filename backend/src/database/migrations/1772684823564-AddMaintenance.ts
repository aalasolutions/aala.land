...
export class AddMaintenance1772684823564 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."work_orders_status_enum" AS ENUM('OPEN', 'IN_PROGRESS', 'PENDING_APPROVAL', 'COMPLETED', 'CANCELLED')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."work_orders_priority_enum" AS ENUM('LOW', 'MEDIUM', 'HIGH', 'URGENT')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."work_orders_category_enum" AS ENUM('PLUMBING', 'ELECTRICAL', 'HVAC', 'STRUCTURAL', 'CLEANING', 'PEST_CONTROL', 'APPLIANCE', 'OTHER')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "work_orders" ADD CONSTRAINT "FK_5709f8a5c8b777a53b406d550ff" FOREIGN KEY ("company_id") REFERENCES "companies"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "work_orders" DROP CONSTRAINT "FK_5709f8a5c8b777a53b406d550ff"`,
    );
