...
export class EnhanceMaintenanceAndDocuments1773500000003 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "property_documents" DROP COLUMN IF EXISTS "uploaded_by"`,
    );
    await queryRunner.query(
      `ALTER TABLE "property_documents" DROP COLUMN IF EXISTS "previous_version_id"`,
    );
    await queryRunner.query(
      `ALTER TABLE "property_documents" DROP COLUMN IF EXISTS "version"`,
    );
    await queryRunner.query(
      `ALTER TABLE "property_documents" DROP COLUMN IF EXISTS "access_level"`,
    );
    await queryRunner.query(
      `ALTER TABLE "property_documents" DROP COLUMN IF EXISTS "category"`,
    );
    await queryRunner.query(
      `DROP TYPE IF EXISTS "property_documents_access_level_enum"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "work_orders" DROP COLUMN IF EXISTS "schedule_frequency"`,
    );
    await queryRunner.query(
      `ALTER TABLE "work_orders" DROP COLUMN IF EXISTS "is_preventive"`,
    );
    await queryRunner.query(
      `ALTER TABLE "work_orders" DROP COLUMN IF EXISTS "cost_notes"`,
    );
    await queryRunner.query(
      `ALTER TABLE "work_orders" DROP COLUMN IF EXISTS "photos"`,
    );
    await queryRunner.query(
      `DROP TYPE IF EXISTS "work_orders_schedule_frequency_enum"`,
    );
  }
}
