...
export class AddAmenitiesToUnits1773200000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!hasColumn.length) {
      await queryRunner.query(
        `ALTER TABLE "units" ADD "amenities" jsonb NOT NULL DEFAULT '[]'`,
      );
    }
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_units_amenities" ON "units" USING GIN ("amenities")`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "units" DROP COLUMN IF EXISTS "amenities"`,
    );
  }
}
