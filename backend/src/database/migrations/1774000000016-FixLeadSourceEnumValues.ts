...
export class FixLeadSourceEnumValues1774000000016 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TYPE "leads_source_enum" ADD VALUE 'WHATSAPP' BEFORE 'whatsapp'`,
    );
    await queryRunner.query(
      `ALTER TYPE "leads_source_enum" ADD VALUE 'REFERRAL' BEFORE 'referral'`,
    );
    await queryRunner.query(
      `ALTER TYPE "leads_source_enum" ADD VALUE 'SOCIAL_MEDIA' BEFORE 'social_media'`,
    );
    await queryRunner.query(
      `ALTER TYPE "leads_source_enum" ADD VALUE 'WALK_IN' BEFORE 'walk_in'`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE "leads" SET "source" = 'whatsapp' WHERE "source" = 'WHATSAPP'`,
    );
    await queryRunner.query(
      `UPDATE "leads" SET "source" = 'referral' WHERE "source" = 'REFERRAL'`,
    );
    await queryRunner.query(
      `UPDATE "leads" SET "source" = 'social_media' WHERE "source" = 'SOCIAL_MEDIA'`,
    );
    await queryRunner.query(
      `UPDATE "leads" SET "source" = 'walk_in' WHERE "source" = 'WALK_IN'`,
    );
    await queryRunner.query(
