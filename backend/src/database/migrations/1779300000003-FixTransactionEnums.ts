...
export class FixTransactionEnums1779300000003 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`UPDATE "transactions" SET "type" = 'INCOME' WHERE "type" NOT IN ('INCOME', 'EXPENSE')`);
    // Ensure category column only has valid TransactionCategory values
    await queryRunner.query(`UPDATE "transactions" SET "category" = 'OTHER' WHERE "category" IS NOT NULL AND "category" NOT IN ('RENT', 'SALE', 'DEPOSIT', 'MAINTENANCE', 'COMMISSION', 'OTHER')`);
    ...
    await queryRunner.query(`ALTER TABLE "transactions" ALTER COLUMN "type" TYPE text USING "type"::text`);
    ...
    await queryRunner.query(`CREATE TYPE "public"."transactions_category_enum" AS ENUM('RENT', 'SALE', 'DEPOSIT', 'MAINTENANCE', 'COMMISSION', 'OTHER')`);
    ...
    await queryRunner.query(`ALTER TABLE "transactions" ALTER COLUMN "type" SET DEFAULT 'INCOME'`);

    // Cast category column from varchar to the new enum (drop default first — PostgreSQL can't auto-cast varchar default to enum)
    await queryRunner.query(`ALTER TABLE "transactions" ALTER COLUMN "category" DROP DEFAULT`);
    await queryRunner.query(`ALTER TABLE "transactions" ALTER COLUMN "category" TYPE "public"."transactions_category_enum" USING CASE WHEN "category" IS NULL THEN NULL ELSE "category"::"public"."transactions_category_enum" END`);
    await queryRunner.query(`ALTER TABLE "transactions" ALTER COLUMN "category" SET DEFAULT 'OTHER'`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE "transactions" ALTER COLUMN "category" DROP DEFAULT`);
    await queryRunner.query(`ALTER TABLE "transactions" ALTER COLUMN "category" TYPE varchar(20) USING "category"::text`);
    ...
    await queryRunner.query(`CREATE TYPE "public"."transactions_type_enum" AS ENUM('rent', 'sale', 'deposit', 'maintenance', 'commission', 'other', 'RENT', 'SALE', 'DEPOSIT', 'MAINTENANCE', 'COMMISSION', 'OTHER', 'INCOME', 'EXPENSE')`);
