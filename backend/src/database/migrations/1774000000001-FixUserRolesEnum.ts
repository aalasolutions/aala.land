...
export class FixUserRolesEnum1774000000001 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "users_role_enum_new" AS ENUM ('super_admin', 'company_admin', 'admin', 'manager', 'agent', 'accountant')`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "users" ALTER COLUMN "role" SET DEFAULT 'agent'::"users_role_enum"`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "users_role_enum_old" AS ENUM ('admin', 'agent', 'manager', 'boss')`,
    );
