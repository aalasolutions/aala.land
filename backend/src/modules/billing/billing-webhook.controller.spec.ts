...
describe('BillingWebhookController', () => {
    ...
    it('forwards an undefined rawBody so the service can reject it', async () => {
        service.handleWebhook.mockRejectedValue(new Error('Missing webhook payload or signature'));
    ...
    it('has no guards on the webhook route (public by design)', () => {
        ...
        const classGuards = Reflect.getMetadata('__guards__', BillingWebhookController);
