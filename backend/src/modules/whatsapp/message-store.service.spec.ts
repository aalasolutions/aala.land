...
describe('MessageStoreService', () => {
  ...
  it('getChatList returns sorted by lastTs desc', () => {
    service.addMessage('user-a', makeMsg({ chatId: 'old', id: 'm1', timestamp: 100 }));
    service.addMessage('user-a', makeMsg({ chatId: 'new', id: 'm2', timestamp: 200 }));
    const list = service.getChatList('user-a');
    expect(list[0].chatId).toBe('new');
  });

  it('chat lastBody reflects newer message only', () => {
    service.addMessage('user-a', makeMsg({ chatId: 'c1', id: 'm1', body: 'first', timestamp: 100 }));
    service.addMessage('user-a', makeMsg({ chatId: 'c1', id: 'm2', body: 'second', timestamp: 200 }));
  ...
  it('getChatList is isolated by userId', () => {
    ...
    expect(service.getChatList('user-a').map(c => c.chatId)).toEqual(['chat-a']);
    expect(service.getChatList('user-b').map(c => c.chatId)).toEqual(['chat-b']);
  });
});
