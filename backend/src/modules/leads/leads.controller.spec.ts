...
describe('LeadsController', () => {
  ...
  const mockReq = {
    user: { companyId, userId: 'user-uuid-1', role: Role.COMPANY_ADMIN },
  };
  ...
  describe('convert', () => {
    it('converts lead to WON', async () => {
      service.convert.mockResolvedValue({ ...mockLead, status: 'WON' } as any);
  ...
  describe('addActivity', () => {
    it('adds activity to lead', async () => {
      ...
      const dto = { type: 'NOTE', description: 'First contact' };
