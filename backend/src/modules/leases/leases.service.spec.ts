...
describe('LeasesService', () => {
  ...
  describe('update', () => {
    ...
    it('throws NotFoundException when lease not found', async () => {
      ...
      await expect(
        service.update('bad-id', companyId, { status: 'EXPIRED' }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('rejects flipping a lease to ACTIVE when the unit already has one', async () => {
      ...
      await expect(
        service.update('lease-uuid-1', companyId, { status: 'ACTIVE' }),
  ...
  describe('renew', () => {
    it('locks the lease, marks it RENEWED, and creates a new lease', async () => {
      const activeLease = { ...mockLease, status: 'ACTIVE' } as Lease;
      const newLeaseData = {
        unitId: 'unit-uuid-1',
        tenantName: 'Ahmed Al-Rashid',
      ...
      manager.save
        .mockResolvedValueOnce({ ...activeLease, status: 'RENEWED' } as Lease)
      ...
      expect(dataSource.transaction).toHaveBeenCalledTimes(1);
      expect(result.oldLease.status).toBe('RENEWED');
      expect(result.newLease).toEqual(savedNewLease);
    });

    it('allows renewal of EXPIRED lease', async () => {
      const expiredLease = { ...mockLease, status: 'EXPIRED' } as Lease;
      const newLeaseData = {
        unitId: 'unit-uuid-1',
        tenantName: 'Ahmed Al-Rashid',
        startDate: '2027-01-01',
      ...
      manager.create.mockReturnValue(newLeaseEntity);
      manager.save
        .mockResolvedValueOnce({ ...expiredLease, status: 'RENEWED' } as Lease)
    ...
    it('rejects the second concurrent renew: status already RENEWED under the lock', async () => {
      ...
      const renewedLease = { ...mockLease, status: 'RENEWED' } as Lease;
    ...
    it('maps the successor unique-index violation to a 400 (two renews, same unit, different old leases)', async () => {
      ...
      await expect(
        service.renew('lease-uuid-1', companyId, { status: 'ACTIVE' } as any),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('throws BadRequestException when lease is TERMINATED', async () => {
      const terminated = { ...mockLease, status: 'TERMINATED' } as Lease;
    ...
    it('throws BadRequestException when lease is DRAFT', async () => {
      const draft = { ...mockLease, status: 'DRAFT' } as Lease;
  ...
  describe('terminate', () => {
    ...
    it('throws BadRequestException when lease is not ACTIVE', async () => {
      const expired = { ...mockLease, status: 'EXPIRED' } as Lease;
