...
describe('getStorageQuotaBytes', () => {
  it('returns FREE_STORAGE_BYTES for FREE tier regardless of seat count', () => {
    const company = makeCompany({ subscriptionTier: SubscriptionTier.FREE, purchasedSeats: 10 });
...
describe('reserveStorage', () => {
  ...
  const makeRepo = (company: Partial<Company> | null, affected: number) => {
    ...
    return {
      findOne:            jest.fn().mockResolvedValue(company ? makeCompany(company) : null),
  ...
  it('resolves and reserves when projected usage is within quota', async () => {
    ...
    await expect(reserveStorage(repo as any, 'company-1', 500)).resolves.toBeUndefined();
  ...
  it('throws 507 and does not reserve when projected usage exceeds quota', async () => {
    const repo = makeRepo(
      { storageUsedBytes: FREE_STORAGE_BYTES as any, subscriptionTier: SubscriptionTier.FREE },
      0,
    );

    await expect(reserveStorage(repo as any, 'company-1', 1)).rejects.toMatchObject({
      status: 507,
    });
  });
  ...
  it('throws 507 exactly at the quota boundary (used + incoming > quota)', async () => {
    const repo = makeRepo(
      { storageUsedBytes: (FREE_STORAGE_BYTES - 10) as any, subscriptionTier: SubscriptionTier.FREE },
      0,
    );

    await expect(reserveStorage(repo as any, 'company-1', 11)).rejects.toMatchObject({
      status: 507,
    });
  });
  ...
  it('resolves when used + incoming equals quota exactly', async () => {
    ...
    await expect(reserveStorage(repo as any, 'company-1', 10)).resolves.toBeUndefined();
  });

  it('throws NotFoundException when company is not found', async () => {
    const repo = makeRepo(null, 1);
    await expect(reserveStorage(repo as any, 'bad-id', 100)).rejects.toThrow('Company not found');
  });
});
