...
// Mutators are bound class fields so templates call them without a per-controller @action wrapper.
export default class ContactSelection {
  ...
  // UI-level requirement; the API keeps these optional so imports and other clients are unaffected.
  get isPresent() {
    return Boolean(this.contact) || this.hasIdentity;
  }

  // Empty when a contact is attached, since the id is sent instead.
  get cleanIdentity() {
