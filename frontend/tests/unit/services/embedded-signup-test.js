...
module('Unit | Service | embedded-signup', function (hooks) {
  ...
  test('marks a plain FINISH as not Coexistence', async function (assert) {
    this.stubLogin({
      data: { phone_number_id: '100000000000001', waba_id: '200000000000002' },
      type: 'WA_EMBEDDED_SIGNUP',
      event: 'FINISH',
    });
    ...
    assert.false(result.isCoexistence);
  });

  test('surfaces the Meta message when CANCEL carries an error', async function (assert) {
    this.stubLogin({
      data: {
        error_message: 'Business portfolio is restricted',
        error_id: '1',
      },
      type: 'WA_EMBEDDED_SIGNUP',
      event: 'CANCEL',
    ...
    try {
      ...
      assert.notOk(error.cancelled);
      assert.strictEqual(error.message, 'Business portfolio is restricted');
    }
  });
