...
export default class DataTableComponent extends Component {
  ...
  @action resetLayout() {
    if (this.args.tableId) this.nuvoStorage.remove(this.layoutKey);
    this.buildColumns();
  ...
  saveLayout() {
    ...
    this.nuvoStorage.set(this.layoutKey, {
      order: this.order,
      hidden: this.hidden,
      widths,
    });
  ...
  restoreLayout() {
    if (!this.args.tableId) return;
    const saved = this.nuvoStorage.get(this.layoutKey);
    if (!saved) return;
  ...
  @action commitEdit() {
    ...
    if (column.editable === 'number') {
      ...
      // Stay in the editor so the bad value is visible and correctable; clearing
      // first discarded the entry silently.
